...
import { restoreApplication } from "./restore-application";
...
describe("restoreApplication", () => {
  ...
  it("redirects unauthenticated users to sign in before accessing the database", async () => {
    ...
    const result = await restoreApplication("application_123");
  ...
  it("only finds an archived application owned by the authenticated user", async () => {
    ...
    await expect(
      restoreApplication("application_123"),
  ...
  it("restores the application and records activity history", async () => {
    await restoreApplication("application_123");
